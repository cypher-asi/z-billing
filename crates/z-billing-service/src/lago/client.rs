...
impl LagoClient {
    ...
    pub fn new(base_url: impl Into<String>, api_key: impl Into<String>) -> Result<Self, LagoError> {
        let client = Client::builder()
            .timeout(Duration::from_secs(30))
            .build()
            .map_err(|e| LagoError::Configuration(format!("Failed to build HTTP client: {e}")))?;
...
mod tests {
    ...
    #[test]
    fn client_creation() {
        let client = LagoClient::new("http://localhost:3000", "test-api-key").unwrap();
