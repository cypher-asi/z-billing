...
pub struct ServiceConfig {
    ...
    /// Admin API key for admin-only endpoints (e.g., adding credits).
    ...
    /// Lago webhook signature secret (optional).
...
impl ServiceConfig {
    ...
    pub fn from_env() -> Self {
        ...
        let (lago_api_url, lago_api_key, lago_organization_id, lago_webhook_secret) =
            load_lago_secrets();
...
fn load_lago_secrets() -> (Option<String>, Option<String>, Option<String>, Option<String>) {
    ...
    (
        ...
        std::env::var("LAGO_WEBHOOK_SECRET").ok(),
    )
}
...
///
/// **Note**: This uses blocking I/O (`std::fs::read_to_string`) and should only
/// be called during startup, before entering the async runtime. This is safe
/// because `ServiceConfig::from_env()` is called synchronously in `main()`
/// before `tokio::main` starts.
fn load_secrets_file<T: serde::de::DeserializeOwned>(path: &str) -> Result<T, std::io::Error> {
    let path_obj = Path::new(path);
    if !path_obj.exists() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::NotFound,
            format!("Secrets file not found: {path}"),
        ));
    }
