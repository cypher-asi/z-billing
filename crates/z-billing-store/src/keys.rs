...
/// # Errors
///
/// Returns an error if the key is not at least 32 bytes or contains invalid ULID bytes.
pub fn extract_transaction_id_from_user_key(
    key: &[u8],
) -> Result<TransactionId, crate::error::StoreError> {
    if key.len() < 32 {
        return Err(crate::error::StoreError::Database(format!(
            "user-transaction key too short: expected 32 bytes, got {}",
            key.len()
        )));
    }
    let mut bytes = [0u8; 16];
    bytes.copy_from_slice(&key[16..32]);
    TransactionId::from_bytes(bytes).map_err(|e| {
        crate::error::StoreError::Database(format!("invalid transaction ID in key: {e}"))
    })
}
...
mod tests {
    ...
    fn extract_transaction_id_roundtrip() {
        ...
        let extracted = extract_transaction_id_from_user_key(&key).unwrap();
        assert_eq!(extracted, tx_id);
    }

    #[test]
    fn extract_transaction_id_short_key_fails() {
        let short_key = vec![0u8; 16]; // Too short
        let result = extract_transaction_id_from_user_key(&short_key);
