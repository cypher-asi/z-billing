...
pub trait Store: Send + Sync {
    ...
    /// # Warning
    ///
    /// This operation is NOT atomic under concurrent access and bypasses the
    /// transaction audit trail. For production use, prefer [`Self::add_credits`]
    /// or [`Self::process_usage`] which provide atomic operations with proper
    ...
    /// Use `add_credits` or `process_usage` instead.
    #[deprecated(
        since = "0.2.0",
        note = "Use `add_credits` or `process_usage` for atomic operations with transaction recording"
    )]
